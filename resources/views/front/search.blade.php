    ...
    <body class="font-poppins text-[#0E0140] pb-[100px] overflow-x-hidden">
        ...
        <section id="Result" class="container max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px] w-fit">
            ...
            <div class="categories-container grid grid-cols-3 gap-[30px]">
                ...
                @empty
                    <p class="text-white text-lg">Data Not Found</p>
                @endforelse
            </div>
                ...
                <div id="Pagination" class="flex items-center justify-center gap-[14px] mt-10">
                    @for ($page = 1; $page <= $jobs->lastPage(); $page++)
                        @if ($page == $jobs->currentPage())
                            <button class="w-[38px] h-[38px] flex shrink-0 rounded-full justify-center items-center
                 text-white font-semibold hover:bg-[#7521FF] transition-all duration-300
                 bg-[#7521FF]">
                                {{ $page }}
                            </button>
                        @else
                            <a href="{{ $jobs->url($page) }}" class="w-[38px] h-[38px] flex shrink-0 rounded-full justify-center items-center
                 text-white font-semibold hover:bg-[#7521FF] transition-all duration-300
                 bg-[#0E0140]">
                                {{ $page }}
                            </a>
