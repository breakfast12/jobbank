...
@section('content')
    ...
    <body class="font-poppins text-[#0E0140] pb-[100px] overflow-x-hidden">
        ...
        <form action="{{ route('front.apply.store', $companyJob->slug) }}" method="POST" enctype="multipart/form-data" class="relative max-w-[900px] mx-auto flex flex-col rounded-[20px] bg-white border border-[#E8E4F8] p-[30px] gap-10 shadow-[0_8px_30px_0_#0E01400D] mt-[130px]">
            @csrf
            ...
            <div id="Resume-Container" class="flex flex-col gap-4">
                ...
                <div class="relative flex rounded-[20px] bg-white ring-1 ring-[#0E0140] p-[14px_24px] gap-[10px] focus-within:ring-2 focus-within:ring-[#FF6B2C] transition-all duration-300">
                    ...
                    <input type="file" name="resume" id="fileInput" class="absolute top-1/2 -z-10" accept=".pdf" required />
                </div>
            </div>
